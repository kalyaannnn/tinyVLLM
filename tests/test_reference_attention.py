...
def test_paged_ref_matches_contiguous() -> None:
    torch.manual_seed(0)

    B, H, D = 3, 2, 8
    block_tokens = 4
    seqlens = torch.tensor([3, 7, 9], dtype=torch.int64)

    # allocate enough physical blocks
    needed_blocks = sum(((int(t.item()) + block_tokens - 1) // block_tokens) for t in seqlens)
