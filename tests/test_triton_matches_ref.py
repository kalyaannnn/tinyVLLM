...
from vllm_scratch.attention.triton_decode import (
    TRITON_AVAILABLE,
...
@torch.no_grad()
def test_triton_matches_ref_decode() -> None:
    ...
    # build block tables
    alloc = BlockAllocator(num_blocks=256)
    ...
    out_ref = paged_attention_decode_ref(q, k_cache, v_cache, block_tables, seqlens, block_tokens)
    out_tri = paged_attention_decode_triton(q, k_cache, v_cache, block_tables, seqlens, block_tokens)
